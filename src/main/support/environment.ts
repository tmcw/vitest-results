...
export function isWindows(): boolean {
	const platform = isNodeJS() ? process.platform : navigator.userAgent;
	return (
		/\bWindows\b/i.test(platform) ||
		/\bWin32\b/i.test(platform) ||
		/\bWin64\b/i.test(platform)
	);
}
...
export function resolvePath(path: string): string {
	const worker = (globalThis as Record<string, unknown>)
		.__vitest_worker__ as Record<string, unknown>;
