...
expect.extend({
    toEqualCloseTo(received: unknown, expected: unknown, precision: number = 3) {
        ...
        const message = (): string => {
            const receivedSummary = this.utils.stringify(received, 1, { maxWidth: 5 });
            const expectedSummary = this.utils.stringify(expected, 1, { maxWidth: 5 });
            return `expected ${receivedSummary} to ${this.isNot ? "not " : ""}deeply equal ${expectedSummary}`
